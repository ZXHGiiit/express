...
Form.prototype.setValidationMsg = function(checkOpt){
    if(checkOpt.lang === "cn"){
        ...
        if(!this.checkOpt.errorText.valueMissing){
            this.checkOpt.errorText.valueMissing = {
                "default": "请填写此项",
                "radio": "请选择其中一项"
            }
        }
        ...
        HTMLInputElement.prototype.checkValidity = function(){
            ...
            for(var x in that.validity){
                ...
                if(that.validity[x]){
                    ...
                    switch(x){
                        case "valueMissing":
                            that.validationMessage = Form.checkOpt.errorText.valueMissing[type] ? Form.checkOpt.errorText.valueMissing[type] : Form.checkOpt.errorText.valueMissing.default;
                            break;
                        case "typeMismatch":
...
*/
    var errMsg = "<p class='" + errorMsgClass + "'><span>" + msg + "</span></p>";
...
Form.prototype.checkInputValidity = function(input){
    ...
    if(!input.checkValidity()){
        ...
        if(errorText && validity.valueMissing && (text = errorText.valueMissing[type] ? errorText.valueMissing[type] : errorText.valueMissing.default)){
            return text;
        }
        ...
        if(!input.value.match(/^[a-z0-9\.\'\-]+@[a-z0-9\-]+\.[a-z0-9\.\-]+$/i)){
            return input.form.Form.validationMessage.email;
        } 
    }
...
/*
    if(navigator.userAgent.match(/iphone|ipad|ipod|android|windows phone/i)){
        $form.on("touchstart", "input", function(event){
            if(this.type === "submit" || this.type === "radio" || this.type === "checkbox"){
                return;
            }
            $form.closest("section").css({"position": "static", "-webkit-transform": "none", "transform": "none"});
            setTimeout(function(){
                this.focus();
            }, 0);
            event.preventDefault();        
        ...
        $form.on("blur", "input", function(event){
            ...
            $form.closest("section").css({"position": "static", "-webkit-transform": "translate(-50%, -50%)", "transform": "translate(-50%, -50%)"});
        });
    }
*/
};
